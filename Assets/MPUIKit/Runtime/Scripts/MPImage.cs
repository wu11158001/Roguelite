...
namespace MPUIKIT
{
    ...
    public class MPImage : Image
    {
        ...
        /// 材質球檢測設定
        /// </summary>
        private void CheckMaterialSettging()
        {
            if (material == null)
            {
                Debug.Log($"[MPImage] 物件：{gameObject.name} 沒有設定材質球", gameObject);
                return;
            }

            ParseMaterialName(material.name);
        }
        ...
        private void ParseMaterialName(string materialName)
        {
            ...
            else if (type.StartsWith("S") && type.Length == 2)
            {
                int stroke = type[1] - '0';
                StrokeChecking(stroke);

                if (int.TryParse(parts[1], out int c))
                    CornerSetting(c);
            }

            var fall = parts.FirstOrDefault(x => x.StartsWith("falloff"));
            if (fall != null && int.TryParse(fall.Substring(7), out int f))
                FalloffChecking(f);
        }
        ...
        /// </summary>
        /// <param name="s"></param>
        private void ParseCorner(string s)
        {
            if (int.TryParse(s, out int corner))
            {
                CornerSetting(corner);
            }
            else
            {
                string letters = new string(s.TakeWhile(char.IsLetter).ToArray());
                string numbers = new string(s.SkipWhile(char.IsLetter).ToArray());

                if (int.TryParse(numbers, out int corner2))
                    CornerSetting(corner2, letters);
            }
        }
        ...
        private void CornerSetting(int corner, string type = "")
        {
            var rect = Rectangle;

            var c = rect.CornerRadius;
            if (string.IsNullOrEmpty(type) || type.Contains("w")) c.w = corner;
            if (string.IsNullOrEmpty(type) || type.Contains("x")) c.x = corner;
            if (string.IsNullOrEmpty(type) || type.Contains("y")) c.y = corner;
            if (string.IsNullOrEmpty(type) || type.Contains("z")) c.z = corner;

            if (rect.CornerRadius != c)
            {
                rect.CornerRadius = c;
                Rectangle = rect;
            }
        ...
        private void StrokeChecking(int stroke)
        {
            if (StrokeWidth != stroke)
                StrokeWidth = stroke;
        }

        private void FalloffChecking(int falloff)
        {
            if (FalloffDistance != falloff)
